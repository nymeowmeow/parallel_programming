...
package object barneshut {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def bound(value : Float, minvalue : Float, maxvalue : Float) : Float = {
        if (value < minvalue) minvalue
        else if (value >= maxvalue) maxvalue - 1
        else value
      }

      val col : Int = ((bound(b.x, boundaries.minX, boundaries.maxX) - boundaries.minX)/(boundaries.width/sectorPrecision)).toInt
      val row : Int = ((bound(b.y, boundaries.minY, boundaries.maxY) - boundaries.minY)/(boundaries.height/sectorPrecision)).toInt
      this(col, row) += b
      this
    }
